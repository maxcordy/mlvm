...
thickness=3; //
//in mm
thickness_diameter=3.56;//
//in mm
height=12; //
//Extrude the name on the interior
int="TRUE";//[TRUE, FALSE]
//Extrude the name on the exterior
ext="TRUE";//[TRUE, FALSE]

/*[Hidden]*/
//Resolution of the circle
$fn=100;
/*  text_on module https://github.com/brodykenrick/text_on_OpenSCAD
...
etiquette(name,radius,thickness, thickness_diameter,height,int,ext);

module etiquette(name,radius,thickness, thickness_diameter,height, int, ext)
//r : rayon du verre
// e : épaisseur du verre
// d : diamètre d'épaisseur du verre
// h : hauteur de l'étiquette
{
...
difference()
{square(size=[thickness+4,height]);
translate([2,0,0])square(size=[thickness,height-2-thickness_diameter/2]);
translate([2+thickness/2,height-2-thickness_diameter/2,0])circle(r=thickness_diameter/2);
}
} 
if(ext=="TRUE")
{rotate(a=4)text_on_cylinder(name,font="Allerta Stencil",r=radius+thickness+4, h=height, extrusion_height=6, size=8, valign="center");}

if(int=="TRUE")mirror([1,0,0])rotate(a=4.5)text_on_cylinder(name,font="Allerta Stencil",r=radius, h=height, extrusion_height=3, size=8, valign="center");
}
rotate(a=
-95-len(name)*3.5/(2*PI*radius)*360)translate([-0,0,0])
cube([radius+thickness+4,radius+thickness+4,height+2]);
rotate(a=
-175+len(name)*3.5/(2*PI*radius)*360)translate([-0,0,0])
cube([radius+thickness+4,radius+thickness+4,height+2]);
}
