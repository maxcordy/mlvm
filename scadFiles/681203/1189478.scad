...
//Size of the sweep as factor of textsize, good start is 1 so it would be as big as 'textsize' or 1.5
text_excenter_factor=1;

//set base height in mm
baseheight=1;

//cut height percent
cut_height_percent=0.7;
height_cut=cut_height_percent*(text_excenter_factor*textsize/2+textsize);

//Where can i read? upper or lower side
direction="up"; //["up", "down"]

//Create a rounded base. Set to 0 for a minimal base
RoundedBase=1; //[0,1]

//Define the round base adder to make the letters meet and not fall apart, as percentage of the textsize
base_radius_add_factor=.2;
base_radius_add=base_radius_add_factor*textsize;

/* [Hidden] */
//-------------------------------------------------------------------------------

cutcube_x=textsize*(len(textstring)+1);
cutcube_yz=textsize*2+text_excenter_factor*textsize;
...
Rotate text 180 degrees and you get some nice shapes.
...
module RoundTextCut(textstr, textsize, direction="up") 
{
    module RoundText(textstr, textsize, direction="up") 
    {
        rotate_extrude(convexity = 10, $fn = 64)
            translate([text_excenter_factor*textsize, 0, 0])
                rotate([0,0,(direction=="up"?-1:1)*90])
                    writetext(textstr, textsize);
    }
    
    module RoundText2DCutoff(textstr, textsize, direction="up") 
    {
        projection(cut = true) 
            rotate([0,90,0])
                translate([-height_cut,0,0])
                    RoundText(textstr, textsize, direction);
    }
    ...
    rotate([0,90,0])
    {
        intersection()
        {
            ...
            //cut out of the full circle
           translate([0,-cutcube_yz,-cutcube_x/2])
                cube([height_cut,cutcube_yz*2,cutcube_x]);
        ...
        if(RoundedBase==0)
        {
            translate([(direction=="up"?-1:0)*baseheight*0+height_cut, 0, 0])
                rotate([0,-90,0])scale([1,(direction=="up"?1:-1),1])
                    linear_extrude(height=baseheight, twist=0, slices=1, $fn=32) 
                        hull()
                             RoundText2DCutoff(textstr, textsize, direction);
        }
        else        
        {
            translate([baseheight+height_cut, 0, 0])
                rotate([0,-90,0])scale([1,(direction=="up"?1:-1),1])
                    linear_extrude(height=baseheight, twist=0, slices=1, $fn=32) 
...
RoundTextCut(textstring,textsize,direction);
