...
// What Wrench (Spanner) sizes?
WrenchSizes = 0;	// [0:Rolson sizes,1:Fairbury sizes]

// Nuts or Bolts?
Type = 0;	// [0:Nuts,1:Bolts]

// What size of metric thread?
Thread = 8;	//[5:100]

// What length of thread (mm)?
Length = 16;	//[5:100]
...
module do_it()
{
	for (ac=[1:Accros])
	{
		...
		{
			translate([
				-(((Accros-1)*Space)/2)+((ac-1)*Space),
				-(((Down-1)*Space)/2)+((dn-1)*Space),
				0])
			{
				if (Type==0)
					hex_nut(Thread);
				else
					hex_bolt(Thread,Length);
			}
		}
	}
}
