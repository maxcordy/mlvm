// <OverKnob for M3 Wingnut> (c) by <arobg> 2016-05-20
// v5
/* the default values create a 33 mm knob which press fits over a M3 wing nut like those used to level the build plate of various 3D printers like the Flashforge Creator Pro.

http://www.thingiverse.com/thing:1552252
...
// http://www.thingiverse.com/thing:1314390
...
//size of metric ISO bolt or nut  M1...M8..M64
headIso=3; // [1:0.1:64]

//height of bolt or nut head, usually around 4/3 the M size
headHeight=4;  // [0:0.1:50]

/* [KnobDimensions] */

//maximum diameter of knob
maxDiameter=32; // [2:0.1:140]

//minimum diameter of knob
minDiameter=25; // [1:0.1:139.9]

//maximum radius chamfer
maxRadiusChamfer=1; // [0.1:0.1:10]

//max height of whole knob
maxHeight=10;  // [5:0.1:140]

//height of holder include chamfers
holderHeight=5 ;    // [5:0.1:100]

//number of side cuts in knob
numberOfCuts=10  ; // [0:30]

//radius of cut
cutRadius=2.9; // [0:0.1:50]

//maxRadius+cutOffset is center of cutting cylinder
cutOffset=0;  // [0:0.1:50]

//big chamfer height
holderChamfer=5;  // [0:0.1:50]

//approximate height of the curve from bottom of a nut or bolt to where the screw threads begin
screwHeadSafetyHeight=0;  // [0:0.1:10]


/* [OtherValues] */

//number of perimeter line segments 8..360
$fn=10;

//somewhere around half the layer width
tolerance=.3;  // [0:0.1:1.0]



module knob(numberOfCuts,maxRadius,minRadius,maxRadiusChamfer,
    maxHeight,holderHeight,cutRadius,cutOffset,holderChamfer,headIso, headHeight,screwHeadSafetyHeight,tolerance, washerDiameter, washerHeight){
        ...
        union(){
            translate([0,0,0]){
                ...
                translate([0,0,maxRadiusChamfer*2]){
                    ...
                    translate([0,0,holderHeight-4*maxRadiusChamfer]){ union(){
                        hull(){
...
// label knob protrusions
            for (knob_label = [0:numberOfCuts-1])
                            text_on_cylinder(t=str(knob_label),r1=maxRadius-maxRadiusChamfer,r2=minRadius-tolerance,h=holderChamfer, font="Liberation Mono", direction="ltr", size=holderChamfer*maxRadius/minRadius,extrusion_height=maxRadius/minRadius+maxRadiusChamfer,eastwest=90-360/numberOfCuts*knob_label,updown=-min(maxRadius/3,holderChamfer)/2,center=false,rounded=true,spacing=.93);
        ...
        //cut for washer/bearing/12L14 Tapped Nut (arobg)
        //diameter of cut for washer or knob insert
washerDiameter=headIso*8/3+tolerance; // [0:0.1:140]

        //height of cut for washer or knob insert
washerHeight=headIso/2+tolerance; // [0:0.1:100]

       translate([0,0,maxHeight-washerHeight])cylinder(d=washerDiameter+tolerance,h=washerHeight);
        
       
        // cut for M3 wing nut (arobg)
        wingReach=headIso*10/3;
        wingSpan=(headIso+tolerance)*2+tolerance;
        wingThickest=headIso*4/3;
        wingThinnest=headIso*2/3;
        wingSqueeze=wingThickest-wingThinnest;
        WingslotPoints = [
            [  -wingReach/2, -wingThickest/2 ,  0 ],  //0
            [ wingReach/2,  -wingThickest/2+wingSqueeze/2,  0 ],  //1
            [ wingReach/2,  wingThickest/2-wingSqueeze/2,  0 ],  //2
            [  -wingReach/2,  wingThickest/2,  0 ],  //3
            [  -wingReach/2,  -wingThickest/2+wingSqueeze/2,  wingSpan ],  //4
            [  wingReach/2, -wingThickest/2+wingSqueeze/2,  wingSpan/1.7 ],  //5
            [ wingReach/2,  wingThickest/2-wingSqueeze/2,  wingSpan/1.7 ],  //6
            [  -wingReach/2,  wingThickest/2-wingSqueeze/2,  wingSpan ]]; //7
        ...
        translate([wingThickest-tolerance,0,maxHeight-wingReach/2+tolerance]) rotate([0,75,0]) polyhedron(WingslotPoints, WingslotFaces);
                translate([-wingThickest+tolerance,0,maxHeight-wingReach/2+tolerance]) rotate([0,75,180]) polyhedron(WingslotPoints, WingslotFaces);
...
// https://github.com/brodykenrick/text_on_OpenSCAD/blob/master/text_on.scad was inserted on May 20, 2016 (arob)
/*  text on ....
...
knob(numberOfCuts=numberOfCuts,maxRadius=maxDiameter/2,minRadius=minDiameter/2,maxRadiusChamfer=maxRadiusChamfer,maxHeight=maxHeight,holderHeight=holderHeight,
