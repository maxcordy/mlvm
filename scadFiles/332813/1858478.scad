...
part = "all"; // [one,two,three,stick,all,stick,printstick,lid]
	...
	color("red")
		translate([(width*2+wall*2)-wall/2,0,-.1])
			rotate([0,0,45])
				...
				{
					translate([0,0,stickh/2+wiggle])
						cube([stickw+2*adder,stickw+2*adder,stickh+wiggle*2],center=true);
...
if (part=="one" || part=="all")
	boxes(num_boxes);

if (part=="two" || part=="all")
