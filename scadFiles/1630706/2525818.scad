// Adapter for using a blowgun to air up the Intex Easy Set pool
// Uses the ISO Metric Thread Implementation by Trevor Moseley
airplug_diameter=18.5;
airplug_height=10;
airplug_quality=96;
blowgun_diameter=10;
blowgun_pitch=2.81/3;
blowgun_length=6;
blowgun_perimeter=1.2; 
blowgun_quality=96;
$fn=96;
...
module blowgun_threads() {
thread_out_centre(blowgun_diameter+1,blowgun_length);
thread_out_pitch(blowgun_diameter,blowgun_length,blowgun_pitch);
}

difference() {
union() {
    airplug_solid();
    translate([0,0,airplug_height])
    blowgun_threads();
}
translate([0,0,-0.5])
cylinder(d=blowgun_diameter-blowgun_perimeter*2,h=blowgun_length+airplug_height+1,$fn=blowgun_quality);
}
