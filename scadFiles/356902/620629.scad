...
linelength=charlength*fontsize;
...
difference () {
	...
	union () {
		translate ([(linelength+4)/2-3,1.5,0])
		cylinder (r=slitwidth/2,h=plate_thickness+0.1,$fn=100);
		translate ([(linelength+4)/2-3,0.5,0])
		cube ([6,slitwidth,plate_thickness+0.1]);
		translate ([(linelength+4)/2+3,1.5,0])
		cylinder (r=slitwidth/2,h=plate_thickness+0.1,$fn=100);
	}

	union () {
		translate ([(linelength+4)/2-3,lineheight-1.5,0])
		cylinder (r=slitwidth/2,h=plate_thickness+0.1,$fn=100);
		translate ([(linelength+4)/2-3,lineheight-2.5,0])
		cube ([6,slitwidth,plate_thickness+0.1]);
		translate ([(linelength+4)/2+3,lineheight-1.5,0])
		cylinder (r=slitwidth/2,h=plate_thickness+0.1,$fn=100);
	}
...
if (text_style>0) {
	translate ([margin+insertlength,0,0])
		...
		union () {
			for (i=[0:textlines-1]) {
			translate ([9,5.25,plate_thickness])
        rotate ([0,180,0])
			write(text[i],t=-0.9,h=fontsize,font=font,space=letterspacing);
			}
}
}