...
//Number of steps which widen the font
steps_bigger=8;

//Number of steps which decrease the font width
steps_smaller=1;

//select base type
basetype="round"; //[round,straight]

//Size increase/reduction in percent of the font size
change_percent=.035;
change=textsize*change_percent;

/* [Hidden] */
//-------------------------------------------------------------------------------
steps=steps_bigger + 1 + steps_smaller;
baseheight=textheight/steps;
...
This is a configurable name plate, where each letter consists of a staggered pyramid.
...
module RiseText(textstr, textsize,textheight) 
{
    module dotext()
    {
        text(textstr,size=textsize,font=fontname,halign="center");
    }
    
    for (z = [0:steps-1]) 
        translate([0,0,baseheight*z])
            linear_extrude(height=baseheight+.01, slices=1, $fn=32,convexity = 32) 
                if (basetype=="round")
                {
                    offset(r =  (steps_bigger-z)*change ) 
                        dotext();
                }
                else
                {
                    offset(delta =  (steps_bigger-z)*change ) 
                        dotext();                   
                }
}
