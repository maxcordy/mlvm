// Thingiverse File Name: Customizable_Slide_Blade_keys.scad
// Belfry Filename: House_key_vBelfry_098.scad
...
//	added new type of key header - for a Keyport Slide Blade
// type of key - 10/07/2013
...
style_of_key = 2; //[0:Straight Header,1:Round (set by polygon count) Header,2:Keyport Slide Blade Header]
//style_of_key = 2;
...
//-------------------------------------------------
//	use this to stop makerbot customizer from showing any more variables
module dummy_module()
{
echo ( "dummy_module" );
}
...
// ----- define the Keyport Slide Blade variables -----

// define an array of slide blade key header lengths (x)
slidebladelength = [17.6,17.6,0.00];	// mm
echo ( "slidebladelength" , slidebladelength );

// define the slide blade key length
slideblade_length = slidebladelength[ type_of_key ];
echo ( "slideblade_length" , slideblade_length );

// define an array of slide blade key header heights (y)
slidebladeheight = [11.5,11.5,0.00];	// mm
echo ( "slidebladeheight" , slidebladeheight );

// define the slide blade key height
slideblade_height = slidebladeheight[ type_of_key ];
echo ( "slideblade_height" , slideblade_height );

// define an array of slide blade key header widths (z)
slidebladewidth = [3.1,3.1,0.00];	// mm
echo ( "slidebladewidth" , slidebladewidth );

// define the slide blade key width
slideblade_width = slidebladewidth[ type_of_key ];
echo ( "slideblade_width" , slideblade_width );

// define an array of slide blade shoulder offset values (x)
slidebladexoffset = [2.80,2.80,0.00];	// mm
echo ( "slidebladexoffset" , slidebladexoffset );

// define the slide blade shoulder offset value (x)
slideblade_xoffset = slidebladexoffset[ type_of_key ];
echo ( "slideblade_xoffset" , slideblade_xoffset );



// ----- end of Keyport Slide Blade variables -----
...
// ----- cut off the end of the slideblade header -------------------------------------------
module cut_the_sbheader()
{
echo ( "cut off the end of the slideblade header module" );

translate( [-2,slideblade_height/2,-1] ) rotate( [0,0,45] ) cube( blade_height *1.5 );

translate( [-2,slideblade_height/2,-1] ) rotate( [0,0,225] ) cube( blade_height *1.5 );

// cut out a notch in the top of the slideblade
// translate( [4.85,slideblade_height-2.30,-1] ) rotate( [0,0,0] ) resize([0,0,5]) cube( 3.0 );

//translate( [4.85,slideblade_height-2.30,0] ) rotate( [0,0,0] ) cube( [3.0,3.0,4.0] );

// cut out a cylinderical hole through the slideblade
translate( [6.35,slideblade_height-2.0,slideblade_width/2] ) rotate( [90,0,0] ) cylinder( h=slideblade_height , r=1.00 , $fn=16);
...
//-------------------------------------------------------------------------
module slide_blade_key_header( sbheader_length = 17.6 , sbheader_text = "0123456789" )
{
echo ( "sbheader_length" , sbheader_length );
echo ( "sbheader_text" , sbheader_text );

translate([(-1*sbheader_length)-(slideblade_xoffset)+fudge_factor,-(slideblade_height/2)+(blade_height/2),0])
{
union()
{

// ----- Keyport slide blade key head setup -----

difference()
{
cube( [ sbheader_length , slideblade_height  , slideblade_width ] );	// slide blade key header

cut_the_sbheader();


translate( [4.85,slideblade_height-2.30,-.5] ) rotate( [0,0,0] ) cube( [3.0,3.0,4.0] );
...
for ( sbdigit_loop = [0:number_cylinders-1] )
{
	echo ( "sbdigit_loop" , sbdigit_loop , "text" , sbheader_text[sbdigit_loop] );

	translate([((text_separation-2.0)+((text_separation-.90)*sbdigit_loop)),3,slideblade_width-fudge_factor]) write(sbheader_text[sbdigit_loop],h=text_h_size-1,t=(layer_thickness*layers_thick));
...
if ( style_of_key == 2 )
{
translate([fudge_factor-blade_height,-(slideblade_height/2)+(blade_height/2),0])
cube([blade_height,(11.5),blade_width]);
} //	end of if
}	//	end of module
...
if ( style_of_key == 2 )
{
// create a Keyport 'slide blade' key header
slide_blade_key_header(sbheader_length=slideblade_length,sbheader_text=str(custom_key_text,key_cut_code));
}	// end of if
...
//----- Keyport slide blade key headers -----

if ( style_of_key == 2 )	//	Keyport slide blade key header
{

	translate([-11,0.75,(layer_thickness*3)]) rotate([0,180,270])
{
