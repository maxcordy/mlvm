...
currency = "rur"; // [usd:USD - US dollar, eur:EUR - Euro, chf:CHF - Swiss franc, cad:CAD - Canadian dollar, rur:RUR - Russian ruble, other:Other (See "Slot customization" tab above)]

// How tall is the the shortest tube? In millimeters:
height = 50; // [10:150]

// Choose a pattern for the back side.
pattern = "chncoin"; // [no:Solid without pattern, chncoin:Chinese ancient coin pattern, mesh:Mesh pattern]

// Would you like Tom-and-Jerry pictures on the left side and right side?
/* Not yet supported */ image = 0; //// [1:Yes please, 0:No thanks]

// Which one would you like to see?
part = "all"; // [all:All these three parts,basebox:Base box only,topboard:Top board only,tubes:Tubes only]
...
rur_coins = [[25.08, 23.17, 22.10, 20.56, 19.47, 17.55],
...
other_coins_d_t = [
  ...
  [coin_9_diameter, coin_9_thickness], [coin_10_diameter, ccoin_10_thickness]];
...
coins = currency == "usd" ? usd_coins :
        ...
        currency == "rur" ? rur_coins :
        currency == "cad" ? cad_coins : other_coins;

coins_d = coins[0];
coins_thickness = coins[1];

enable_box = (part == "all" || part == "basebox");
  enable_dovetail = true;
  enable_box_emptier = false;
  enable_mesh = (pattern != "no");
  enable_image = (image == 1);
enable_top_board = (part == "all" || part == "topboard");
enable_tubes = (part == "all" || part == "tubes");
...
unassembled_top_board_lift = 20;
unassembled_tubes_lift = 10;
...
module main() {
  ...
  if (enable_top_board) {
    translate([0, 0, assembled ? 0 : unassembled_top_board_lift]) {
      top_board();
    }
  }
  if (enable_tubes) {
    translate([0, 0, assembled ? 0 : unassembled_tubes_lift]) {
      tubes();
    }
  }
}
...
module board_back_hollow(thicker=0) {
  assign(xyz = box_size(fatter = -board_thickness - thicker))
...
module board_left_hollow(fatter=0, thicker=0) {
  assign(xyz = box_size(fatter = -board_thickness - fatter))
...
module image_left() {
  translate([0, (board_width - image_left_size) / 2,
    ...
    resize([image_left_size, image_left_size, 1]) {
      surface(file = "TomHead_gray.1.dat", convexity = 2);
    }
...
module image_right() {
  translate([board_length - 1,
    ...
    resize([image_right_size, image_right_size, 1]) {
      surface(file = "Jerry_bw.dat", convexity = 2);
    }
...
module board_back_mesh_grids(
  ...
  assign(length = (thickness + gap) * (count - 1) + thickness) {
...
module slope_cut_for_coin(n, bigger_r=0) {
  render(convexity=2) {
    intersection() {
      ...
      transform_top_board(sorter_min_height) {
        assign(a = board_primary_slope, th = board_thickness)
        assign(cut_length = coins_d[n] * 1.5)
        assign(move_x = coin_left_x(n+1) - cut_length)
        assign(cut_length_rotated = cut_length / cos(a))
        assign(move_x_rotated = move_x / cos(a) + th * tan(a) + 1) {
...
module tube_stronger() {
  for (i = [0 : coin_max_index]) {
    assign(x = coin_center_x(i))
    assign(r = coins_d[i] / 2 + coin_padding) {
...
module tube_scales() {
  for (i = [0 : coin_max_index]) {
    render(convexity=2)
    assign(interval = (coins_thickness[i] > 1.5 ? 1 : 2))
    for (j = [interval : interval : sorter_max_height/coins_thickness[i]]) {
      translate([coin_center_x(i),
        ...
        assign(length_offset = (j%10==0 ? 3 : j%2==0 ? 1 : -0.5))
