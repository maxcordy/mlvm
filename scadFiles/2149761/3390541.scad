// This is a customizer Remix of http://www.thingiverse.com/thing:8925 by nrp.  
// I added the "credit card" and the ability to enter your numbers without using SCAD.  Everything else is original work from original design.
...
// First measurement
First_measurement = 1; //[1:7]
// First measurement
Second_measurement = 1; //[1:7]
// First measurement
Third_measurement = 1; //[1:7]
// First measurement
Fourth_measurement = 1; //[1:7]
// First measurement
Fifth_measurement = 1; //[1:7]
...
module kw1(bits) {
    ...
    // Adding the credit card to the key
    difference(){

    // The Card
    translate([0,4]){
        linear_extrude(height=1)offset(r=5)
    square([75, 45], center=true);
                    }
    // The empty space in the card
       
    translate([-21.5,-6]){
        linear_extrude(height=1)offset(r=3)
    square([49, 21]);
                         }
                }
...
kw1([First_measurement,Second_measurement,Third_measurement,Fourth_measurement,Fifth_measurement]);
