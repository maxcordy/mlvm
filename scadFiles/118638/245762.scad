...
m_dist = die_width/5.2; 

// Do you want the corners rounded off?
rounded = 1; //[1:True,0:False]

// Do you want the floating face ?
floating  = 1; //[1:True,0:False]
floating_space = 1.25;  //[0.5:3]
...
side_1_mode = 0; //[0:Dots,1:Characters]
side_1_characters = "!";
side_1_size = 4; 
side_1_rotation = 45;
side_1_font = "Letters.dxf";
//["Letters.dxf":Basic,"orbitron.dxf":Orbitron,"BlackRose.dxf":BlackRose]
	...
	union(){
		if(rounded){
			if(floating){
				difference(){
					intersection(){
						cube(size=die_width,center=true);
						sphere(r=die_width*.7,center=true);
					}
					die_cube_cut();
				}
				die_cube_bone();
...
module die_cube_cut(){
	cylinder(r=die_width/2-floating_space,h=die_width+.1,center=true);
	rotate ([90,0,0])cylinder(r=die_width/2-floating_space,h=die_width+.1,center=true);
	rotate ([0,90,0])cylinder(r=die_width/2-floating_space,h=die_width+.1,center=true);
}

module die_cube_bone(){
cylinder(r=die_width/2-floating_space*2,h=die_width,center=true);
	rotate ([90,0,0])cylinder(r=die_width/2-floating_space*2,h=die_width,center=true);
	rotate ([0,90,0])cylinder(r=die_width/2-floating_space*2,h=die_width,center=true);
}
