...
intersection () {
	...
	difference () {
		...
		translate([height/2,-120,-1]) cylinder(r=150-15,h=thick+2);
// Grip Text
if (bold=="Yes") {translate([height/2,-120,thick]) writecircle(text,[0,0,0],150-15/2,t=4,h=8, space=1.1);}
else {translate([height/2,-120,thick]) writecircle(text,[0,0,0],150-15/2,t=4,h=8, space=1);}
if (bold=="Yes"){translate([height/2,-120,thick]) rotate ([0,0,.2])writecircle(text,[0,0,0],150-15/2,t=4,h=8, space=1.1);}
}
}
if (raise=="Yes") {
...
// Top clip
translate([0,-34,0])cube([3,48,thick]);
difference() {
	translate([0,-30,0])cube([6,6.5,thick]);
	translate([3,-53,thick/2])rotate ([0,90,0]) cylinder(r=27,h=5);
}
intersection() {
	translate([0,-34,0])cube([6,9,thick]);
	translate([0,-53,thick/2])rotate ([0,90,0]) cylinder(r=25,h=10);
}

// Bottom clip
translate([height-3,-86,0])cube ([3,82,thick]);
...
difference() {
	translate([height-9,-86,0])cube ([9,10,thick]);
	translate([height-11.9,-53,thick/2])rotate ([0,90,0]) cylinder(r2=26,r1=cand/2,h=9);
}

intersection() {
	translate([height-7,-77,0])cube ([7,50,thick]);
