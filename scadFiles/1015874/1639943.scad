...
module flange() {
	flange_hole_pos = Base_diameter/2 - flange_hole_offset;
	difference() {
		translate([-Base_diameter/2, -Base_diameter/2, 0])
			cube(size = [Base_diameter, Base_diameter, Flange_thickness]);
		translate([0, 0, -Delta])
			cylinder(h = Flange_thickness+Delta*2, d = Base_diameter-Wall_thickness*2);
		// Holes
		translate([flange_hole_pos, flange_hole_pos, -Delta])
			cylinder(h = Flange_thickness+Delta*2, d=Flange_hole_dia);
		if (Quarter_only != "yes") {
