//Parametric Build Plate Support for Delta Printer
...
Wing_OD = 310;      //310mm original
                    //If using Outside Ledge, you probably want to add 10mm or more

//2. Diameter of Heat Bed Hole Mounting Pattern
HeatBd_Hole_OD = 300;   //I Rotate 1st Hole 3 degrees From Y-axis to miss side connection
...
MidLedge_offset = 110;          //110 is good starting point for 310mm OD

//24. Draw Spoke Ledges (1 = Yes, 0 = No)
DrawSpokes = 1;

//25. How many Ledge spokes to use
Spokes = 5;         //5 will give you 1 Ledge spoke every 30 degrees
...
{
    //translate([0,Wing_Rad2-14.7+Y_Half,0])
    translate([0,0,0])
    rotate([0,180,0])
    color("Blue",.5)
    import("BuildPlate_4HoleOvalBracket_rev22.stl", convexity=3);
}
module Center38mmHoleBracket()
{   
    translate([0,0,0])
    rotate([0,180,0])
    color("Blue",.5)
    import("BuildPlateCenter38mmHolePattern_Circle_Bracket_rev2_repaired.stl", convexity=3);
}
...
if (Wing_Ang == 30 || Wing_Ang == 60 || Wing_Ang == 90 || Wing_Ang == 120)
{
    if (Assembled == 0)
    {
        if (DrawBars == 0)
    ...
    echo("********************************************");
    echo("**** Wing_Ang must be 30, 60, 90 or 120 ****");
    echo("********************************************");
}