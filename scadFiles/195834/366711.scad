...
draw_electrons(or, ir, ring_height, ring_height_step, ring, ring_width, element, atomic_number, electrons_on_each_ring);
...
module draw_electrons(or, ir, ring_height, ring_height_step, ring, ring_width, element, atomic_number, electrons_on_each_ring)
{
number_of_electrons_on_ring1=electrons_on_each_ring[0];
number_of_electrons_on_ring2=electrons_on_each_ring[1];
number_of_electrons_on_ring3=electrons_on_each_ring[2];
...
if (ring==0 && ( number_of_electrons_on_ring1>0)) {
	...
	writecube(element,[0,0,0.3*(ring_height+(2*ring_height_step*ring))],[0,0,0],face="top", t=font_depth, h=8, up=center_radius/2,font=font); 
	else
	writecube(element,[0,0,0.3*(ring_height+(2*ring_height_step*ring))],[0,0,0],face="top", t=font_depth, h=8, up=center_radius/3,font=font); 

	//Write the atomic number
	writecube(atomic_number,[0,0,0.3*(ring_height+(2*ring_height_step*ring))],[0,0,0],face="top", t=font_depth, h=8, down=center_radius/1.5,font=font); 
}
...
for (electron = [1 : number_of_electrons_on_ring1])
	rotate(a=360/number_of_electrons_on_ring1*electron, v=[0,0,1])
		translate(v=[0.5*(or+ir), 0, 0.5*(ring_height+(2*ring_height_step*ring))])
			difference() {
				sphere(r=(ring_width/2)-0.5, center=true, $fn=20);
