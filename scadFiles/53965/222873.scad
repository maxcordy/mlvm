...
//ugly modded for yegah gear generation by yru

ydia=15;
%cylinder(r=ydia/2,h=1); //div dia mockup cus yru doesn't like funky module values, I prefer to leave some space for mount correction or recompile //makieta srednicy podzialowej, poniewaz yru nie lubi nierownych wartosci modulu, woli zostawic troche miejsca montazowego na regulacje odleglosci osi.

ptch=1.5; //tmodule //modol zebow
Ath=round(ydia/ptch); //n/o teeth //ile zebow
...
mirror([0,0,0]) rotate([180,0,0])
difference(){
union(){

#translate([0,0,-.025])
	gear (number_of_teeth=Ath,
			circular_pitch=180*ptch,
			hub_diameter=Ath*ptch*1.25,
			hub_thickness=0,
			rim_width=1,
			bore_diameter=1.1,
			rim_width=2,
...
translate([0,0,.1])
	gear (number_of_teeth=Ath,
			...
			bore_diameter=1,
			rim_width=2,
			gear_thickness=totgt/2,
			twist=gtw,
			circles=5,
clearance = .4,
	backlash=bcksl,
involute_facets=ivfc,
			rim_thickness=totgt/2);
...
translate([0,0,totgt/2+3-.05]) cylinder(r=ydia/(3.14/2),h=6,center=true,$fn=26);


}
#translate([0,0,totgt/2+3-.05]) cylinder(r=5.1/2,h=32,center=true,$fn=26);
translate([0,0,totgt/2+3-.05]){
hull(){
translate([0,5,0]) rotate([90,0,0]) rotate([0,0,30]) cylinder(r=6.2/2,h=2.8,center=true,$fn=6);
#translate([0,5,2]) rotate([90,0,0]) rotate([0,0,30]) cylinder(r=6.6/2,h=2.8,center=true,$fn=6);
}
#translate([0,9.6,0]) rotate([90,0,0]) cylinder(r=3/2,h=14,center=true,$fn=16);
}
}
