//Tolerance, adjust according to your printer.
t = 0.6;
//select piece
piece= "all"; //[connector,male,female,all]
...
//radius of FSR
fsrR = 6.5;
fsrPos = 40-11.76+fsrR/2;
//radius of cogwatch sensor
electronicR = 41;
outerR= electronicR + wall*2 + t; 
//height of cogwatch sensor
electronicH = 15.5;
topH = electronicH+wall+t;
ThreadSize=2.5;
/*[Hidden]*/
//variables for connector
connectorR = 15;
CThread = 1;
...
module connector(){
	union(){
		rotate([180,0,0]) male_interface(t=t);
		metric_thread((connectorR-t)*2,
                    CThread, wall,
                    internal=false);
	}
}
...
module top(){
	union(){
		difference(){
   	    cylinder(r=outerR,h=topH, $fn=75);
      	 translate([0,0,wall]) 
        	 metric_thread((electronicR+wall+t)*2,
                    ThreadSize, topH-wall,
                    internal=true, n_starts=2);
                 metric_thread(connectorR*2,
                    CThread, wall,
                    internal=true);
   	}
	}	
...
module bottom(){
   ...
   difference(){
		union(){
       cylinder(r=outerR,h=wallB);
       translate([0,0,wallB])
        difference(){
            metric_thread((electronicR+wall)*2, ThreadSize, electronicH, n_starts=2);
            cylinder(r=electronicR,h=electronicH, $fn=75 );
        }
		}
       for (a = [0:360/n:360]){
           rotate([0,0,a])
            translate([fsrPos,0,0]) cylinder(r=fsrR,h=wall);
       }
		 for (a = [0:360/64:360]){
           rotate([0,0,a])
            translate([-outerR,0,0]) cylinder(r=0.75,h=wall, $fn=3);
       }
    }
}
...
if (piece == "all"){
bottom();
translate([outerR*2+wall,0,0])
 	top();
translate([outerR+wall,outerR+wall*2,6])
    connector();
}
if (piece == "connector"){
connector();
