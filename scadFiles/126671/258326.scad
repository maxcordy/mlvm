...
parts_to_render = 6; // [1:head, 2:body, 3:arm, 4:leg, 5:base, 6:preview_assembled, 7:preview_exploded, 8:all]
...
// top line of text
text1 = "This IS the droid";
//bottom line of text
text2 = "you are looking for";
...
module body() {
	difference(){	
		...
		// holes for legs
		translate([14.7,0,-28]) cylinder(r=8.4,h=20);
		translate([-14.7,0,-28]) cylinder(r=8.4,h=20);
...
module head() {
	rotate([0,180,0]){ 
	union(){
		translate([0,0,0-5]) cylinder(r=29, h=16);
		translate([0,0,-5]) cylinder(r=31, h=2);
		difference(){
			sphere(r=35);
			cylinder(r=35,h=35);
		...
		translate([-20,0,-48]) rotate([0,15,0]) cylinder(r=3,h=20);
		translate([20,0,-48]) rotate([0,-15,0]) cylinder(r=3,h=20);
		translate([20,0,-48]) sphere(r=3,h=20);
		translate([-20,0,-48]) sphere(r=3,h=20);	
		}
	}
}
...
module leg() {
	union(){ 
		cylinder(r=8,h=leg_h);
		translate([0,0,leg_h]) cylinder(r=6.5,h=6);
		translate([0,0,leg_h]) rotate_extrude(convexity = 10) translate([5.5,0,0]) circle(r=2.5);
	}
}
