...
//thread_out_centre(14,16);				// make a centre for an M8 x 16 ISO thread
//thread_out_pitch(14,16,2.1);			// make an M8 x 16 thread with 1 mm pitch
//thread_out_centre_pitch(8,16,0.5);	// make the centre for an M8 x 16 thread with 1 mm pitch
//thread_in(14,10);						// make an M8 x 10 ISO thread
//thread_in_ring(14,10,3);				// make a ring to enclose an M8 x 10 ISO thread with thickness 2 mm
//thread_in_pitch(14,10,2.1);			// make an M8 x 10 thread with 1mm pitch
...
//  dia=diameter, 6=M6 etc.
{
	...
	difference()
	{
		...
		//FIX Added 0.1 tolerance to cylinder r
	}
...
//  thr=thread quality, 10=make a thread with 10 segments per turn
{
	...
	for(sg=[0:thr-1])
		...
		//FIX remove 0.1 tollerance from Rmin
}
...
//  sh = segment height (z)
{
	...
	ae = as + s ;		// angle to end of seg (no overlap)
	z = sh*sg;
	//pp = p/2;
	//   1,4
	//   |\
