...
module verticalcoins(numcoins,str)
{
    ...
    for(y = [max-(numslots-1):max+(remainder!=0?1:0)])
        translate([(y%2==0?coin_dist/2:0)+coin/2+5,
                    y*coin_5_thick+19-(y>max?coin_5_thick*(5-remainder)/5+.01:0)-(remainder!=0?coin_5_thick*remainder/5:0),
                    coin+20])
        {
                rotate([90,0,0])
                    cylinder(r=(coin+wiggle)/2,h=(y>max?coin_5_thick*remainder/5:coin_5_thick));
            ...
            rotate([coin_rotate,0,0])
                difference()
                {
                    ...
                    translate([-2,0,0])
                    {
                        for(x = [0:3])
                        {    
                            translate([x*(coin+coin_dist),0,0])
                            {
                                //verticalcoins(1,(x>1?3:6));
                                #verticalcoins(numcoinslots[x],txt_arr[x]);
                                if (x==3)
                                {
                                    //the 2 dice slot
                                    #translate([dice*0.2,24,coin+20])
                                        cube([dice*2,dice,dice],center=true);
                                }
                            }
                        }
                        //the logo
...
if (part=="coins" || part=="all")
{
    translate([0,-coin_length-(l_arr[0]+2.5*wall),0])
        ...
        {
            coinholder();
            //uncomment to look into the coin holder for debugging purposes
            //translate([40,0,0]) cube(80);
        }
}

//show the german machi koro box 
//%translate([0,-box_h,0]) cube([box_w,box_h,box_depth]);
