...
module geoMicroLid()
{
	difference()
	{
		...
		translate([0,0,0])
			cylinder(r1=id+(2*walls),r2=id+walls,h=2*walls,$fn=fn2,center=true);
	}
...
module geoMicroBase()
{
	difference()
	{
		...
		translate([(3*id)+(2*walls),0,walls])
			difference()
			{
				cylinder(r2=id+(2*walls),r1=id+walls,h=2*walls,$fn=fn2,center=true);
				cylinder(r2=id+3*walls/2,r1=id+walls/2,h=2*walls,$fn=fn2,center=true);
			}
