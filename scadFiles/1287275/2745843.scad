...
//Future attempts at catching orphaned geometry
// [Catching Abandoned Geometry] */
//How many lost geometries do you have? ("i", "j", "!", "?", etc)
CatchThemAll = 0;
//Change the size of each net.
OrphanSizes = [
    [4,4],
    [4,4],
    [4,4],
...
module WriteText (textstring){ //Write text, centered at 0,0, and correct i/j dots.
    ...
    text(textstring,size=FontSize,font=FontName,halign="center",valign="baseline");
}
//
module HomesForOrphans(textstring){
    ...
    intersection(){
        ...
        linear_extrude(height=1,convexity=10)offset(delta=-OrphanSize)offset(delta=OrphanSize)text(textstring,size=FontSize,font=FontName,halign="center",valign="baseline");
        
        //connect only vertical parts by shifting
        linear_extrude(height=1,convexity=10){
            translate([0,OrphanSize,0])text(textstring,size=FontSize,font=FontName,halign="center",valign="baseline");
        translate([0,-OrphanSize,0]) text(textstring,size=FontSize,font=FontName,halign="center",valign="baseline");
        }
    }
}
...
module BaseBar(linetext,linenum,lowertext=""){ //Pass the text for line 1 and autogenerate bar
    hull(){ 
        intersection(){
            linear_extrude(height=FontExtrude, convexity=10)
            text(linetext,size=FontSize,font=FontName,halign="center",valign="baseline");
    ...
    if(linenum>1){
        hull() {
            intersection(){ //Top of left post
                ...
                translate([-FontSize*FontSpace/2,-(FontSize*LineSpace),0]) cube([1,FontSize*(LineSpace)*2,1]);
            }
            intersection(){ //Bottom of left post
                ...
                text(lowertext,size=FontSize,font=FontName,halign="center",valign="baseline");
