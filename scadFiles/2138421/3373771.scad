...
/* [Anet A8 Pulley Parameters] */

// Length of toothed part of pulley (default: 6.5)
pulley_t_ht = 6.5; // [1:0.1:15]

// Should belt retainer be added next to the teeth?
retainers = 1; // [0:No, 1:Yes]

// Height of belt retainers (default: 1.5, WARNING: Keep this value always above 'Notch Depth')
retainers_ht = 1.5; // [0.1:0.1:5]



// Depth of notch into the retainers in order to fit the bearing brims (default & highly recommended: 1 - fits the bearings perfectly)
notch_depth = 1; // [0:0.1:5]

// Should support for retainer notch be added? (No further support for print needed, default: yes)
add_support = 1; // [0:No, 1:Yes]

/* [Parameters standard pulley lib] */
...
// Anet A8 Pulley Insert (added by TNS)
rotate([180,0,0])  // Rotate for support on bottom
union(){
difference(){
pulley ( "GT2 2mm" , GT2_2mm_pulley_dia , 0.764 , 1.494 );

translate([0,0,-50])cylinder(h = 100, r = motor_shaft/2);
translate([0,0,pulley_t_ht+retainers_ht-notch_depth])cylinder(h = notch_depth + 1, r = 12/2);
translate([0,0,-1-retainers_ht])cylinder(h = notch_depth + 1, r = 12/2);
    
    
}

if(add_support && !(add_support && !retainers)){
difference(){
translate([0,0,pulley_t_ht+retainers_ht-notch_depth])cylinder(h = notch_depth, r = 10.6/2);

translate([0,0,pulley_t_ht+retainers_ht - notch_depth - 0.1])cylinder(h = notch_depth + 0.2, r = 9/2);
}
}
}
