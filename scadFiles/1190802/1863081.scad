...
// Modified by o462 December 2015 (added D-shaft + corrected captive nut disabling)
...
d_shaft = 1;        // D shaped shaft, 0 = No, 1 = Yes
d_shaft_depth = 0.5;  // mm of the cut part from the shaft
...
//	translate ([0,0, pulley_b_ht + pulley_t_ht + retainer_ht ]) rotate ([0,180,0])
	...
	difference()
	 {	 
        ...
        // D shaft
    if(d_shaft && no_of_nuts <= 1) {
        if(d_shaft_depth > motor_shaft / 3) {
            echo ("WARNING: D SHAFT BIGGER THAN THIRD OF MOTOR SHAFT");
        }
        intersection() {
            cylinder(r=motor_shaft/2,h=pulley_b_ht + pulley_t_ht + retainer_ht * retainer,$fn=motor_shaft*4);
            translate([motor_shaft / 2 - d_shaft_depth, -motor_shaft/2, 0]) cube([d_shaft_depth,motor_shaft,pulley_b_ht + pulley_t_ht + retainer_ht * retainer]);
        };
    }
