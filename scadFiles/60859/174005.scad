...
module rotatedCup() {
  ...
  if (cup_hex_nut) {
    rotate_extrude($fn = fn) polygon([
      ...
      [outscribeR-2.5, shankInterfaceWidePoint[1] + (part == "one_piece" ? 12: 10)],
...
module print_part() {
  // include <4G.scad>;
  if (part == "two_piece") {
    translate([0,  0,  7 ]) rotatedCup();
    if (print_support) {
      translate([0,  0,  7 ]) support();
    }
    translate([rim_diameter + 5,  0,  0 ])
    rotate([  0,       180,      0])
      rotatedShank();

  } else if (part == "one_piece") {
    translate([0,  0,  -pShank[6][1]+2 ]) rotatedCup();
    translate([0,  0,  -pShank[6][1]+2 ]) rotatedShank();
    if (print_support) {
      translate([0,  0,  -pShank[6][1]+2 ]) raft();
    }
