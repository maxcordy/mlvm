...
// How tight you want the bar to fit. in mm of airgap.
tolerance = 1.0;

// Letter or number to put on the side
character = "\u2318";

// Feet for bolting it to a surface
screw_feet = "no"; // [yes,no]
...
module side(height, width, thickness, tolerances)
{
  bar_height = 10;
  bar_width = 8+tolerances;
  tab_width = 2-tolerances;
  tab_thickness = 2-tolerances;
  feet_height = 30;
  feet_holes_diameter = 5;
  feet_thickness = 8;
  feet_width = width/8;
  ...
  difference()
  {
    union()
    {
      ...
      if(screw_feet == "yes")
      {
        difference()
        {
          cube([feet_width, feet_thickness, feet_height]);
          translate([feet_width/2, 0, thickness+(feet_height-thickness)/2])
            rotate([270,0,0])
              cylinder(d=feet_holes_diameter, h=feet_thickness+2);
        }
      }
    }
    translate([width/2 - bar_width/2, height-bar_height, tab_thickness])
      cube([bar_width, bar_height, thickness]);
    translate([width/2 - (bar_width-tab_width*2)/2, height-bar_height, 0])
      cube([bar_width-tab_width*2, bar_height, thickness]);
    translate([width/2,0-width/3,0])
      cylinder(d=width, h=thickness);
  
    polyhedron(
      points=[ [0,0,0],[width/2 - bar_width/2 -5, height, 0],[0, height, 0], 
               [0,0,feet_height],[width/2 - bar_width/2-5, height, feet_height],[0, height, feet_height] ],
      faces=[ [0,2,1],[0,1,3],[1,4,3],[1,2,4],[2,5,4],[2,0,5],[0,3,5],[3,4,5] ]
    );
    
    translate([width/2,height/2-height*0.05,0])
      cylinder(d=height/3, h=thickness);
  }
}
render()
{
  difference()
  {
    union()
    {
      side(holder_height,holder_width, 10, tolerance);
      translate([holder_width, 0, 0]) mirror([1,0,0]) side(holder_height,holder_width, 10, tolerance);
    }
    translate([holder_width/2, holder_height-holder_height/2/2, 8])
      linear_extrude(height=2) text(character, font=font, valign="center", halign="center", size = holder_height/7);
    //cube([120, 70, 10]);
  }
}
