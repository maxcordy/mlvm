...
// The first line of text to use
Text1="Local";
// The second line of text to use
Text2="Pub";
// Size of letters
TextHieght=55; // [20 : 170]
...
// Type of coin to use
CoinType = "EU"; // [US:US 1 Cent, CA:CA 1 Cent, EU:EU 2 Cent, UK:UK 1 Penny, XX:Manual]
// Manual coin radius
ManualRadius=9.5;
...
lineheight = (hieght/2);// the hieght of the first line 
  ...
  difference() {
    union() {
      rotate([0,0,90]) translate([-0.5*length, -0.5*base,0]) {
		...
		if (relief>0) translate([ypos,xpos-lineheight,thickness+relief/2]) write(Text1,t=relief,h=hieght,center=true,font=Font,space=spacing);
if (relief>0) translate([ypos,xpos-3.3*lineheight,thickness+relief/2]) write(Text2,t=relief,h=hieght,center=true,font=Font,space=spacing);
    }
