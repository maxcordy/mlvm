...
offset = 33;  // Same as DELTA_EFFECTOR_OFFSET in Marlin. (center to rod centerline)
mount_radius = 12.5;  // Hotend mounting screws, standard would be 25mm.
hotend_radius = 8.25;  // Hole for the hotend (J-Head diameter is 16mm) 
push_fit_height = 6;  // Length of brass threaded into printed plastic. may need to minimize to 3mm, (height-push_fit_height=hotend recess distance)
...
{
    rotate([0, 0, a])
    translate([0, offset+13, bracket_height/2])
    rotate([180, 0, 0])
    bracket();
}
*/
...
rotate([0, 0, 60])
translate([0, 0, eff_height/2])
effector();

for (i = [0:120:359])
{
    rotate([0, 0, i])
    translate([0, -dual_oc_spacing/2, bracket_height/2+eff_height+rim_height_top-eff_recess-bracket_recess])
    bracket();
}
...
{
    rotate([0, 0, j])
    translate([0, -dual_oc_spacing/2, heatsink_total_height+eff_height-eff_recess])
    rotate([180, 0, 0])
    Jhead();
}
...
module effector() {
	...
	//rotate([0, 0, 30]) translate([offset-2, 0, 0])
	  //cube([10, 15, eff_height], center=true);
    ...
    rotate([0, 0, 90])  //rotate dual mount around centerline
        {
        for (k = [0:120:359])
        {
            rotate([0, 0, k])
            //Hotend #1
            translate([dual_oc_spacing/2, 0, 0])
            {
            //push fit hole
...
module bracket()  {
    color( "aquamarine", 1)
    difference() {
      cylinder(r=bracket_offset-3, h=bracket_height, center=true);
        union()  {
            ...
            //trim bracket ends for tri head
            rotate([0, 0, -5]) //---------------------------------------------------
            translate([-bracket_offset, 0, -bracket_height])
            cube([bracket_offset, bracket_offset, bracket_height*2]);
            
            //end trim
            translate([-bracket_offset, bracket_offset/2+1, -bracket_height])
            cube([bracket_offset*2, bracket_offset, 15]);
            //hotend recess
