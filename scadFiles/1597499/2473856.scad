/*
Customizable headphone shell with central thread
2013-11-10 - kowomike
uses Thread library: http://www.thingiverse.com/thing:8793 by syvwlch
*/
...
part = "both"; // [container:Container only,lid:Lid only,both:Container and Lid]

//Inner headroom
inner_height=16; //[15:50]

//Inner diameter of container
inner_diameter=50; //[40:100]

//Text to be engraved in lid
text="Your text here!";

//Font for text on lid
font="write/orbitron.dxf"; //[write/orbitron.dxf:Futuristic,write/Letters.dxf:Basic,write/knewave.dxf:Round,write/BlackRose.dxf:Classic,write/braille.dxf:Braille]
...
module print_part() {
	if (part == "container") {
		...
		Container();
		translate ([inner_diameter+5,0,inner_height+4])
		rotate ([180,0,0])
		Lid();
	}
}
...
union () {
	...
	polygon(points=[[0,2],[radius-1,2],[radius,3],[radius,inner_height-1],[radius+0.9,inner_height-1],[radius+0.9,inner_height-3],[radius+2,inner_height-3],[radius+2,3],[radius+1,1],[radius,0],[0,0]] );


difference () {
cylinder (r=6.5,h=inner_height+1,$fn=100);

trapezoidThreadNegativeSpace(
	length=inner_height+4,				// axial length of the threaded rod
	pitch=4,				// axial distance from crest to crest
	...
	stepsPerTurn=30			// number of slices to create per turn
		);
}
...
difference () {
	rotate_extrude(convexity=10,$fn=100)
	polygon(points=[[0,inner_height+2],[radius-1,inner_height+2],[radius,inner_height+1],[radius,inner_height-1],[radius+1.1,inner_height-1],[radius+1.1,inner_height-3],[radius+2,inner_height-3],[radius+2,inner_height+1],[radius+1,inner_height+3],[radius,inner_height+4],[0,inner_height+4]] );


writecircle(text,[0,0,inner_height+4.5-0.5],inner_diameter/2-fontsize/(11/10),t=1,font=font,space=1.1,h=fontsize);
