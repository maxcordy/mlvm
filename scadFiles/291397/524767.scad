...
// Updated by Gorgar to make it easier to get off the build plate
//  I added a texture on bottom and one edge with a bevel.
...
module bottom() {
  difference() {
    RRect(length-4,width-4,radius-edge/2,1);
    for (i=[0:3:length]) {
       translate([i-(length-4)/2,0,0]) cube([1,200,20],center=true);
    }
  }
}

//color("blue") RRect(length-4,width-4,radius-edge/2,1);
...
render(32) {
  difference() {
    ...
    union() {
      bottom();
      translate([-length/2-1,0,0]) rotate([90,0,0]) rotate([0,0,40])
      cube([4,5,width+1],$fn=3,center=true);
    }
  }
}