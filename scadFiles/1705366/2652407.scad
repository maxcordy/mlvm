...
hub_diameter = 25;
bore_diameter = 10;
hub_height_multiplier = 3;
gear_thickness_multiplier = .75;
double_gear_yes_or_no = "yes";

if (double_gear_yes_or_no != "yes"){
    height = height /2;
}
	...
	difference()
	{
		union ()
		{
			...
			if (flat == false && hub_thickness > gear_thickness)
				translate ([0,0, -hub_thickness/2 + height/4 + height/8])
...
test_double_helix_gear(pressure_angle,holes,teeth,height,gear_twist,circular_pitch,clearance,rim_width,hub_diameter,bore_diameter,hub_height_multiplier,gear_thickness_multiplier,double_gear_yes_or_no);
...
module test_double_helix_gear ()
{
	//double helical gear
	{
		...
		gear (number_of_teeth=teeth,
			...
			pressure_angle=pressure_angle,
			clearance = clearance,
			gear_thickness = height/2*gear_thickness_multiplier,
			rim_thickness = height/2,
			rim_width = rim_width,
			hub_thickness = height/2*hub_height_multiplier,
