...
1.4
 - 	added ellipse_intersection.scad code for 2 new cross section shapes
...
//*******User*Variables*******
/* [Hidden] */ 
//how smooth and round is it? Bigger numbers mean smoother shapes but take a lot longer to render! Setting this value will also cause how the jewel is rendered to be changed from a proper 57/58 facet round brilliant cut.
$fn=50; 
...
//enter US numerical ring size
ring_size_us_can=6.5;//[1:100]
//Enter UK numerical ring size, make sure to convert from letters to numbers i.e. a=1, b1/2=2.5, z=26
//	ring_size_uk_ie_aus_nz=12.5;[a:1 to z:26 in half sizes]
...
//difference between ring inner and outer radius
main_wall=2;//[1:5]
//how wide the band of the ring is
main_depth=4.34;//[1:20]
//this is the 3rd variable for the cross section, it is not used in simple shapes like rectangular but controls features for other shapes including scallop depth, chamfer length, fillet radius, make sure it is less than half of the smallest of the 2 previous parameters
main_feature=0.2;//[0:5]
...
cs_shape="cs_rounded_ellipse_intersection";//[cs_ellipse, cs_rectangle, cs_pill, cs_hemi_ellipse, cs_scallopped_rectangle, cs_rounded_scallopped_rectangle, cs_rounded_hemi_ellipse, cs_rounded_rectangle, cs_chamfered_rectangle, cs_half_pill, cs_reverse_hemi_ellipse, cs_reverse_rounded_hemi_ellipse, cs_half_chamfered_rectangle, cs_ellipse_intersection, cs_rounded_ellipse_intersection]
	...
	// cs_ellipse_intersection=0;
	// cs_rounded_ellipse_intersection=1;
...
module ring_engraving(internal_diameter=20, wall=5, depth=5, text_inside=false, text_itself="Some Text", text_height, text_depth=1, text_spacing=1.2, text_font="write/orbitron.dxf"){
	...
	if (text_inside==true){
		mirror(1,0,0)writecylinder(text_itself,[0,0,-depth/2],internal_radius,depth,rotate=0,t=text_depth,h=text_height,space=text_spacing,font=text_font);
	}else{
...
module ring_shape(ring_internal_diameter, ring_depth, ring_wall, ring_feature){
	rotate_extrude (convexity = 20) translate([(ring_internal_diameter)/2, 0, 0]){
		...
		else if(cs_shape=="cs_reverse_hemi_ellipse==1){
			translate (v=[ring_wall,0,0]) rotate(a=[0,0,180]) hemi_ellipse_shape (ring_depth, ring_wall);	
		}
		else if(cs_shape=="cs_reverse_rounded_hemi_ellipse==1){
