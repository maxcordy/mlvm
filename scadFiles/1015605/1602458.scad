//Added Round Belt Pulleys to Bodged Customizer
//9/15/15

//Customizable Vee Pulley with Nut Retainers
//Bodged together by Jimtoolman
//September 13, 2015
...
number_of_teeth = 0; // [10:350]

profile = 16; // [1:MXL, 2:40DP, 3:XL, 4:H, 5:T2.5, 6:T5, 7:T10, 8:AT5, 9:HTD_3mm, 10:HTD_5mm, 11:HTD_8mm, 12:GT2_2mm, 13:GT2_3mm, 14:GT2_5mm, 15:Vee_Groove, 16:Round_Belt]

// (in mm)
motor_shaft_diameter = 16.0;

// (in mm)
toothed_part_length = 5; // [3:30]

// (in mm)
base_height = 10; // [0:30]

// (in mm)
base_diameter = 40;	 // [10:550]

/* [Idler & retainer] */

belt_retainer = 0; // [0:No, 1:Yes]

retainer_height = 1.5; // [0.5:Tiny (0.5mm), 1:Small (1mm), 1.5:Normal (1.5mm), 2:Big (2mm), 3:Bigger (3mm), 4:Hudge (5mm)]

belt_idler = 0; // [0:No, 1:Yes]

// Height of idler flange over pulley. Set to same as base height if you want an idler but no pulley.
idler_height = 1.5; // [0.5:Tiny (0.5mm), 1:Small (1mm), 1.5:Normal (1.5mm), 2:Big (2mm), 3:Bigger (3mm), 4:Hudge (5mm)]

/* [Holding] */

screws_disposition = 2; // [0:None, 1:1 screw, 2:2 screws at 90°, 3:3 screws at 120°, 4:4 screws at 90°]
...
/* [V Pulley] */
// Total thickness of the vee pulley
//base_height = 14; 
...
edgeThickness = 1;//[1:10]

pulleyHeight = ropeDiameter+edgeThickness*2;

pulleyDiameter = 75;//[20:250]
...
nut_width = nut_widths[screw_profile] + 0.4;
nut_depth = nut_depths[screw_profile] + 0.5;
...
Vee_groove_pulley_dia = tooth_spacing (0,0);
Round_Belt_pulley_dia = tooth_spacing (0,0);
...
if ( profile == 15 ) { pulley ( "Vee Groove" , Vee_Groove_pulley_dia , 0 , 0 ); }
if ( profile == 16 ) { pulley ( "Round Belt" , Round_Belt_pulley_dia , 0 , 0 ); }
// Functions
...
//	************************************************************************
	...
	translate ([0,0, base_height + toothed_part_length + retainer_height + base_height/4
     ]) rotate ([0,180,0])
	...
	difference()
	 {	 
		union()
		{
			...
			if ( base_height < 2 ) { echo ("CAN'T DRAW PULLEY BASE, HEIGHT LESS THAN 2!!!"); } else {
				rotate_extrude($fn=base_diameter*2)
				{
						square([base_diameter/2-1,base_height]);
						//square([base_diameter/2,base_height-1]);
						//translate([base_diameter/2-1,base_height-1]) 
                        //circle(1);
                    ;
				}
			}
		...
		difference()
			...
			if ( profile == 15 ) { Vee_Groove();}
            if ( profile == 16 ) { Round_Belt();}
            }
		...
		if ( base_height < nut_width ) { echo ("CAN'T DRAW CAPTIVE NUTS, HEIGHT LESS THAN NUT DIAMETER!!!"); } else {
		if ( (base_diameter - motor_shaft)/2 < nut_depth + 3 ) { echo ("CAN'T DRAW CAPTIVE NUTS, DIAMETER TOO SMALL FOR NUT DEPTH!!!"); } else {
...
module Round_Belt()
{
	...
	difference()
	{
        ...
        cylinder(r = pulleyDiameter/2, h = pulleyHeight, center = [0, 0, pulleyHeight/2]);
		rotate_extrude(convexity = 10) 
		translate([pulleyDiameter/2, pulleyHeight/2 , 0]) 
		circle(r = ropeDiameter/2);

		// This is the hole for the shaft
		cylinder(r = motor_shaft/2, h = pulleyHeight+100, center = [0, 0, 0]);
	}
}
