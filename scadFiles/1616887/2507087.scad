...
bladeD = 102.5;/*blade diameter mm*/
bladeT = 0.9;/*blade thickn mm*/
lineDur = 2; /*total time in between slits ms*/
blankTime = 1.8;/*time in covered area ms*/
...
difference(){
  ...
  for (a =[1:slitNum]){
    rotate([0,0,(a*(360/slitNum))]){ 
        slit(bladeD*0.99,bladeT,slitSize);
    difference(){
        slit(bladeD*0.99,bladeT,slitLineDur);
...
module mount_holes(){
    cylinder(h=10,d=7,center=true,$fn=100);
    //cylinder(h=5,d=21,center=true,$fn=3);
    translate([16,0,0]){
        //text("1",5);
