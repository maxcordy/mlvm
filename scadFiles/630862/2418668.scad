 ...
 *  G�n�rateur de palonnier de servo pour OpenScad
 *
 *  Copyright (c) 2012 Charles Rincheval.  All rights reserved.
 *
 *  Modified to add support for 2F size 2015 Paul Barrett.
 *  Modified to add support for 1F size 2016 Paul Barrett.
 *
...
// Clear between arm head and servo head. With PLA material, use clear : 0.3, for ABS, use 0.2
SERVO_HEAD_CLEAR = 0.3;

// Servo hub size
hub_type = 2; // [3:3F-25T, 2:2F-22T, 1:1F-15T]

taper_amount = 3; // [3:Full, 2.5:Medium, 2:None]
...
module servo_standard(length, count) {
      if (hub_type == 1)
          servo_futaba_1f(length, count);
      else if (hub_type == 2)
          servo_futaba_2f(length, count);
		else
        ...
        difference() {
            union() {
                ...
                linear_extrude(height = head_heigth) {
                    polygon([
                        [-tooth_width / 2, 0], [-tooth_width / taper_amount, tooth_length],
                        [tooth_width / taper_amount, tooth_length], [tooth_width / 2, 0]
                    ]);
                }

                translate([0, tooth_length, 0]) {
                    cylinder(r = tooth_width / taper_amount, h = head_heigth);
                }
