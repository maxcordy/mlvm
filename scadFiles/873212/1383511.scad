...
// extra PCB is 20x33.7

part = "top"; // [ top, bottom, button, buttons, all ]
with_battery = 1; // [ 1:true, 0:false ] 
extra_room = 20.6; //20.6; // size of extra room, if any, set to 0 otherwise

have_usb_plug = 1; // [ 1:true, 0:false ] 
have_power_plug = 0; // [ 1:true, 0:false ] 
have_power_switch = 1; // [ 1:true, 0:false ] 
have_up_key = 0; // [ 1:true, 0:false ] 
have_down_key = 0; // [ 1:true, 0:false ] 
have_left_key = 0; // [ 1:true, 0:false ] 
...
have_trimmer_hole = 0; // [ 1:true, 0:false ]

/* [Hidden] */
batt = [17.0, 53.0, 26.0]; // 
pcb = [80.1, 57.9, 1.45]; // lcd/keypad shield, net
pcb2floor = 17.0;
pcb2roof = 10.8;
lcdpcb2roof = 7.0;
...
lcdpcb = [pcb[0], 36.0, 1.5]; // size
lcdframe = [71.2, 24.2, 3.8]; // net size
window = [64.0, 14.0]; // size
windowdx = (4.2 - 4.6)/2;
windowdy = (19.6 - 14.0)/2 + 0.5; // 0.5 is fudge, don't ask
pcbmntdia = 3.1;
pcbmntdx = (72.15 + 3.05)/2; // distance between lcd mounting holes
pcbmntdy = (28.05 + 3.05)/2; 
pcbmnt2dia = 3.5;
...
powersy = 9.0; // power contact width
...
module top() {
	...
	module sub() {
       ...
       button_hole(-button_dx1, 0, have_left_key); // left
		...
		translate(trimmer) {
			cylinder(r = trimmer_dia/2 + tol, h=pcb2roof+wall+(have_trimmer_hole ? d : -breakaway), $fn=16);
		}
       // extra room 
